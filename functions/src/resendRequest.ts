/**
 * Mirror of `src/lib/contact/resendRequest.ts`. Duplicated by necessity: the
 * functions package is a separate deploy unit (firebase ships only `functions/`),
 * so it can't import from the Next app's `src/`. Keep the two in sync — they are
 * intentionally identical pure logic.
 */
