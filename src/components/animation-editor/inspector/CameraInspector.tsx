...
import type { CameraKeyframe, BezierPreset } from "@/lib/animation/types";
  ...
  return (
    <div className="space-y-1">
      ...
      <div className="border-t border-zinc-100 pt-3 mt-3">
        <h4 className="text-[11px] text-zinc-500 mb-2">Depth Fade</h4>
        <NumberInput
          label="Near"
          value={keyframe.depthNear}
          onChange={(v) => onChange({ depthNear: v })}
          min={0.1}
          max={10}
        />
        <NumberInput
          label="Far"
          value={keyframe.depthFar}
          onChange={(v) => onChange({ depthFar: v })}
          min={0.1}
          max={10}
        />
      </div>
